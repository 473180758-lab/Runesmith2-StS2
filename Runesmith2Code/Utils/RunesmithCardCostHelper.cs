...
public static class RunesmithCardCostHelper
{
    public static CardCostColor GetElementsCostColor(Runesmith2Card card, ICombatState? state)
    {
        ...
        if (card.Tags.Contains(RunesmithTags.Recipe) && !card.HasElements()) return CardCostColor.InsufficientResources;
        ...
        out Elements hookModifiedCost)
    {
        ...
        (var isModified, hookModifiedCost) = RunesmithHook.TryModifyElementsCost(state, card, hookModifiedCost);
